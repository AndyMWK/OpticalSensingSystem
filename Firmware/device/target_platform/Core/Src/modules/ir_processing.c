...
static ema_lpf_cache_t ema_mem = {.pd1_adc_prev = 0, .pd2_adc_prev = 0};
...
void stream_from_fifo_ema_lpf(fifo_t* fifo_pd1, fifo_t* fifo_pd2, data_msg_t* data_msg)
{
    ...
    adc_msg[PHOTODIODE_1].adc_value =
        (float) SMOOTHING_FACTOR_EMA * (float) ema_mem.pd1_adc_prev +
        (1.0f - (float) SMOOTHING_FACTOR_EMA) * (float) adc_msg[PHOTODIODE_1].adc_value;
    adc_msg[PHOTODIODE_2].adc_value =
        (float) SMOOTHING_FACTOR_EMA * (float) ema_mem.pd2_adc_prev +
        (1.0f - (float) SMOOTHING_FACTOR_EMA) * (float) adc_msg[PHOTODIODE_2].adc_value;
...
static inline float convert_to_distance(uint16_t adc)
{
    return (float) GAIN * sqrtf((float) COEFF_A / (((float) adc / 4095.0f) * (float) VREF)) +
           (float) OFFSET;
}

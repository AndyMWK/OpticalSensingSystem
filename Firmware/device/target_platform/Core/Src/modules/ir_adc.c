...
void adc_conversion_complete_callback(IrAdcContext_t* context, fifo_t* fifo_pd1, fifo_t* fifo_pd2,
                                      uint32_t* buffer)
{
    // check for any null pointers
...
static void pack_adc_msg(IrAdcContext_t* context, adc_msg_t* msg, photodiode_t sensor)
{
    ...
    uint32_t timestamp =
        ((uint32_t) time.Hours * 3600U) + ((uint32_t) time.Minutes * 60U) + time.Seconds;

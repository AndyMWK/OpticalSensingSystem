...
static fsm_context_t ctx = {.state = STATE_IDLE, .error_count = 0};
...
static ir_led_pwm_t ir_led_pwm = {.pwm_duty_cycle = 50, .target_duty = 50};
...
/// @param async_signal asynchronous signal that indicates to other modules that FSM output needs to
...
void update_fsm(fifo_t* fifo_pd1, fifo_t* fifo_pd2, comms_msg_t* comms)
{
    ...
    data_msg_t incoming_fifo_data = {.distance_pd_1 = 0.0,
                                     .distance_pd_2 = 0.0,
    ...
    switch (ctx.state)
    {
        ...
        case STATE_ERROR:
            append_log_msg_to_uart(
                "Maximum Error Count Reached. Device Restart Recommended...\r\n");
            break;
        ...
        case STATE_INTERNAL_BUFFER_ERROR:
            append_log_msg_to_uart(
                "Internal Message Buffers uninitialized. Device Restart Recommended...\r\n");
            break;
...
void output_from_fsm(fsm_output_types_t out_type, char* message, uint16_t* filled_len,
                     const uint16_t message_max_size)
{
...
/// @brief creates a log message based on incoming sensor data. The log message is meant to be
/// printed on the serial console.
/// @param incoming_fifo_data   packaged fifo data containing adc data, timestamps, and sensor
/// statuses
static void create_log_msg(data_msg_t* incoming_fifo_data)
{
    ...
    snprintf(output_msg.uart_tx + used, sizeof(output_msg.uart_tx) - used,
             ...
             (unsigned long) (distance_pd_1_cm / 100U), (unsigned long) (distance_pd_1_cm % 100U),
             (unsigned long) incoming_fifo_data->timestamp_pd_1, (unsigned int) PHOTODIODE_2,
             (unsigned long) (distance_pd_2_cm / 100U), (unsigned long) (distance_pd_2_cm % 100U),
...
/// @brief increments the sensor error count until max threshold is reached. Once the max error
/// count is reached,

...
void pop_log(UART_HandleTypeDef *huart) {
    ...
    // Unless using DMA this is a blocking function. Using this blocking version of the serial transmission is alright but DMA is preferred to reduce CPU usage. 
    HAL_UART_Transmit(huart, (uint8_t*)&log_buffer[log_buffer_pop_ptr], strlen(log_buffer), HAL_MAX_DELAY);

...
int main(void)
{
  ...
  HAL_UART_Receive_DMA(&huart1, rs485_buffer, 4);
  ...
  while (1)
  {
    ...
    if(adc_tick) {
      adc_conversion_complete_callback(&ir_adc_context, &fifo_buffer, adc_buffer);
      adc_tick = 0;
    }
    ...
    // default task to just drain the data into serial.
    pop_log(&huart2);
